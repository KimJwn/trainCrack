        ...
        # 처음 방문한 좌표라면
            ...
            if direction_dict[key] in start_interval_point_list:
                ...
                while True:
                    ...
                    #pdb.set_trace()
                    end_point = [next_key for next_key in possible_key if direction_dict[next_key] in start_interval_point_list]
                    #pdb.set_trace()
                    if end_point:
                        ...
                        next_point = [next_key for next_key in possible_key if img[tuple(direction_dict[next_key])] == 255]
                        #if end_point:
                        #    print(row, col)
                        if len(next_point) > 1:
                            #pdb.set_trace()
...
def renewal_profiling_crack_width_func(img, img_th, total_segment_list, total_chain_list):
    ...
    for j in range(len(total_chain_list)):
        ...
        for i in range(0, len(segment_block)):
            ...
            segment_width_block.append(total_width) #min(total_width, max_width)
    ...
    for segment_block in total_chain_list:
        ...
        if len(segment_block) > 7 :
            for i in range(1, len(segment_block)):
                p_width = n_width = 0
                start = segment_block[0]
                positive_d, negative_d, pc, pf, nc, nf = direction_func(direction=segment_block[i], LorR=-1)  # type: ignore
                row = start[0]
                col = start[1]
                ...
                while True :
                    ...
                    if img[n_next_pixel[0], n_next_pixel[1]] == 255: 
                        ...
                        if img_th[n_c[0], n_c[1]] == 255 | img_th[n_f[0], n_f[1]] == 255 | img_th[n_next_pixel[0], n_next_pixel[1]] == 255: 
                            break
...
def adaptive_crack_width_func(img, total_segment_list):
    ...
    circular_mask_cache_zip_list = list(zip(range(MIN_CIRCULAR_MASK_RADIUS_RANGE, MAX_CIRCULAR_MASK_RADIUS_RANGE), mask_list, mask_area_pixel_num_list))
...
def normal_crack_width_func(img, total_segment_list, radius=20):
    ...
    for segment_block in total_segment_list:
        ...
        for row, col in segment_block:
            ...
            if row - radius < 0 or row + radius > img.shape[0] - 1 or col - radius < 0 or col + radius > img.shape[1] - 1:
                width_list.append(0)
                continue
            else:
                crack_area_pixel_num = masking_circular_area(img, row, col, radius, mask)
