...
#       1-1. 딥러닝 마스크 불러오기 - 흑백 영상 
        ...
        if th_-msk_th < 0 :     m = 0
        elif th_-msk_th < 10 :  m = 7
        elif th_-msk_th < 20 :  m = 15
        elif th_-msk_th < 30 :  m = 20
        else :                  m = 5

        if msk_th < 31 :    th = 40
        elif msk_th < 65 :  th = 50
        elif msk_th < 60 :  th = 55
        elif msk_th < 75 :  th = 70
        elif msk_th < 85 :  th = 75
        else : th = 80
...
#        3-0. 흰색 노이즈 제거
        ...
        if th <= 50 :      img = cv2.GaussianBlur(img, (1, 1), 0)
        elif th <= 75 :    img = cv2.GaussianBlur(img, (3, 3), 0)
        elif th <= 80 :    img = cv2.GaussianBlur(img, (5, 5), 0)
        else :  img = cv2.GaussianBlur(img, (7, 7), 0)
        ...
        if th < 45: th, otsu_img = threshold_otsu_func(img,th=th) #otsu_func(img)
        else :      th, otsu_img = threshold_otsu_func(img,th=th-m-15) #otsu_func(img)
...
#        5-1. 노이즈 제거 영상과 딥러닝 마스크 and 연산
        ...
        if msk_th < 75 : 
            img_and_mask = cv2.GaussianBlur(img_and_mask, (7, 7), 1)
            ret2, img_and_mask = threshold_otsu_func(img_and_mask,th=220) #otsu_func(img)
        else : 
            img_and_mask = cv2.GaussianBlur(img_and_mask, (7, 7), 1)
            ret2, img_and_mask = threshold_otsu_func(img_and_mask,th=245) #otsu_func(img)
...
#        7-4. 균열 선분 이미지에 분기점 시각화하기
        ...
        img_length_interval_point = color.display_crack_color(img_BGR, total_segment_list, total_length_list, mode='interval_point')
        cv2.imwrite(each_save_dir + '{0}_7_1_full_length_interval_point.jpg'.format(img_name), img_length_interval_point)
...
#        usable width func : normal, adaptive, profiling_1 profiling_2
        if args.width_func == 'adaptive':
            ...
            total_width_list, img_pro = renewal_profiling_crack_width_func(img_and_mask, img_thinned, total_segment_list, total_chain_list)
        else :
...
#        8-3. 균열 이미지에 폭을 색상으로 시각화하기
        img_full_width = fill_crack_width_func(img_and_mask, img_thinned, total_segment_list, total_chain_list, total_width_list)
